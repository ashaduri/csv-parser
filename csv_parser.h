...
class CsvParser {
	public:
		...
		/// Default: true.
		constexpr void useEmptyCellType(bool use_empty_cell_type);

		/// Check whether an empty cell type is a separate type from (empty) string.
		[[nodiscard]] constexpr bool useEmptyCellType() const;


		/// Parse CSV string data and store the result using a callback function.
